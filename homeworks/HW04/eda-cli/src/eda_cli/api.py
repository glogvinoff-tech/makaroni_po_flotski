...
@app.post(
    ...
    description=(
        "Принимает CSV-файл, запускает EDA-ядро и возвращает полный набор флагов качества, "
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    ...
    Возвращает все флаги, включая новые эвристики из HW03.
    """
    ...
    # Получаем все флаги, включая булевы и числовые
    all_flags = {}
    for key, value in flags_all.items():
        if isinstance(value, (bool, int, float, str)):
            all_flags[key] = value
        elif hasattr(value, '__dict__'):
            # Если это объект, преобразуем его в словарь
            all_flags[key] = value.__dict__
        else:
            # Иначе преобразуем в строку
            all_flags[key] = str(value)
    ...
    print(
        ...
        f"flags_count={len(all_flags)} latency_ms={latency_ms:.1f} ms"
    )
    ...
    return {
        ...
        "dataset_shape": {"n_rows": n_rows, "n_cols": n_cols},
        "flags": all_flags,
        "processing_time_ms": float(round(latency_ms, 2))
    }