...
### Обоснование неиспользования Dataset 04

Датасет S07-hw-dataset-04.csv не был выбран для анализа по следующим причинам:

1. **Содержит категориальные признаки** - в отличие от датасетов 01-03, которые состоят только из числовых признаков
2. **Требуется дополнительная предобработка** - необходимо применять OneHotEncoder или другие методы кодирования категориальных переменных
3. **Сложность сравнения** - использование разных типов признаков усложняет прямое сравнение алгоритмов кластеризации
4. **Основная цель** - исследование поведения классических алгоритмов кластеризации (KMeans, DBSCAN, Agglomerative) на чисто числовых данных

Для обеспечения консистентности экспериментальной установки было принято решение использовать только датасеты с числовыми признаками.
...
Проверялась устойчивость KMeans на **Dataset A (S07-hw-dataset-01.csv)** при изменении `random_state` (5 запусков с разными seed).

**Эксперимент по устойчивости:**
- Тестируемый алгоритм: KMeans
- Датасет: Dataset A (S07-hw-dataset-01.csv)
- Число кластеров: k=2 (оптимальное значение для данного датасета)
- Число повторений: 5
- Значения random_state: [42, 123, 456, 789, 999]
- Метрика сравнения: Adjusted Rand Index (ARI)

**Результаты:**
- ARI scores: [0.998, 0.999, 1.000, 0.999, 1.000]
- Средний ARI: 0.999 ± 0.001
- Минимальный ARI: 0.998
- Максимальный ARI: 1.000

**Анализ устойчивости:**
1. **Высокая стабильность:** Все значения ARI > 0.998, что указывает на практически идентичные разбиения
2. **Надёжность инициализации:** Параметр `n_init=10` гарантирует, что алгоритм запускается с разных начальных центроидов и выбирает лучшее решение
3. **Чёткая структура данных:** Чёткие границы между кластерами обеспечивают повторяемость результатов
4. **Устойчивость к случайности:** Несмотря на стохастическую природу алгоритма, результаты крайне стабильны

**Вывод:** KMeans демонстрирует очень высокую устойчивость на Dataset A. Это делает его надёжным выбором для прикладных задач, где важна воспроизводимость результатов.
...
6. **Устойчивость подтверждает качество:** высокий ARI между разными запусками говорит о надёжности найденного решения. Проверка на Dataset A показала отличную устойчивость KMeans (ARI > 0.998).
...
**Обоснование неиспользования OneHotEncoder:** В данной работе не использовался датасет 04, содержащий категориальные признаки, поэтому применение OneHotEncoder не требовалось. Все три выбранных датасета (01-03) содержат только числовые признаки, что обеспечило консистентность экспериментальной установки и позволило сосредоточиться на сравнении алгоритмов кластеризации без дополнительных преобразований типа данных.